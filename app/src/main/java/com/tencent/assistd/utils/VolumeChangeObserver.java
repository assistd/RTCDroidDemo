...
public class VolumeChangeObserver {
    ...
    public void registerReceiver() {
        Log.i(TAG, "registerVolumeReceiver");
    ...
    public void unregisterReceiver() {
        Log.i(TAG, "unregisterVolumeReceiver");
        if (mRegistered) {
            try {
    ...
    private static class VolumeBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            Log.i(TAG, "vol change type: " + intent.getIntExtra(EXTRA_VOLUME_STREAM_TYPE, -1) + " action: " + intent.getAction());
            //媒体音量改变才通知
            if (VOLUME_CHANGED_ACTION.equals(intent.getAction())
                    && (intent.getIntExtra(EXTRA_VOLUME_STREAM_TYPE, -1) == AudioManager.STREAM_MUSIC)) {
                VolumeChangeObserver observer = mObserverWeakReference.get();
                Log.i(TAG, "vol change observer != null: " + (observer != null));
                if (observer != null) {
                    VolumeChangeListener listener = observer.getVolumeChangeListener();
                    Log.i(TAG, "vol change listener != null: " + (listener != null));
                    if (listener != null) {
                        int volume = observer.getCurrentMusicVolume();
                        Log.i(TAG, "vol change a " + volume);
                        if (volume >= 0) {
